...
- Performance gate: text `run_fps=86.15`; sprites `run_fps=70.28`; sprites with headless audio output `run_fps=61.10`, `apu_dropped_samples=0`.
- Super Mario Land action gate: headless action `run_fps=73.90`; action with headless audio output `run_fps=62.55`, `apu_dropped_samples=0`. Latest live action capture checked 10 non-startup profile windows with min `wall_fps=46.84`, min queue `33.5 ms`, and zero audio underruns/drops.
...
- Crystal PyBoy CGB visual oracle: static 60/600/2400/3600/4800 wall-frame comparison passes with 30 PNG artifacts, including targeted crops. Dynamic `--scenario dynamic` now adds 11 title animation, menu movement, text, clock, and confirmation checkpoints through frame 7800; all 11 current dynamic checkpoints have `diff_ratio=0.0000`, `major_diff_ratio=0.0000`, `nonblack_delta_ratio=0.0000`, `visible_mismatch_class=none`, and `mismatch_class=none`. Saved-game `--scenario overworld` loads `saves\pokemon-crystal-test.sav`, freezes GBemu and PyBoy to `saves\pokemon-crystal-test.sav.rtc` `saved_at=1779714125.565259`, and covers checkpoints 4800, 5400, 7200, 8400, 8580, 9060, 9600, 10200, 10800, and 11400. Frames 4800, 8580, 9060, 9600, 10200, and 10800 are pixel-exact. Frames 5400, 7200, 8400, and 11400 have small transient `obj_priority`/OBJ coverage diffs of 75-78 pixels with nonblack coverage delta at or below `0.0017`; BG and OBJ palette RAM match PyBoy at every source-debug checkpoint. The focused 5400/8400/9600 probe shows the remaining visible focus-frame drift is shadow-OAM tile timing: frame 5400 first OAM diff is tile byte `100` vs `96` at offset `0x22`, and frame 8400 first OAM diff is `104` vs `100` at offset `0x12`; frame 9600 is exact. Completed-frame and PPU-input-clock probes matched one early shadow-OAM tile group but worsened the full frame-5400 comparison, so the default oracle remains on CPU-cycle capture with wall-frame input. The earlier saved-game palette drift was the RTC/time-of-day baseline, not RGB555 conversion or palette-register behavior. Frame-3600 source-debug still reports an invisible bank-0 source-state drift, but no visible mismatch.
