...
- CGB header detection for CGB-enhanced and CGB-only ROMs, explicit `--mode dmg|cgb|auto`, forced CGB mode for CGB-only cartridges, CGB post-boot CPU identity basics, Crystal first-frame/window-startup smoke coverage, staged Crystal CGB render-gate coverage, and a guarded CGB foundation for `FF4F` VRAM banking, `FF70` WRAM banking, `FF51`-`FF55` GDMA/HDMA VRAM DMA, `FF68`-`FF6C` palette/OPRI registers, KEY1 `FF4D` STOP-triggered double-speed switching, first-pass CGB BG/window rendering with palette attributes, tile VRAM-bank attributes, X/Y flips, and first-pass CGB OBJ rendering with OBJ palette attributes, tile VRAM-bank selection, OAM/OPRI object ordering, BG priority attributes, LCDC.0 priority behavior, and RGB framebuffer output.
...
The oracle runs GBemu and PyBoy in CGB mode at 60, 600, 2400, 3600, and a scripted 4800-frame title/menu checkpoint using the same `frame:buttons[:duration]` input-script format. It writes GBemu, PyBoy, and amplified diff PNGs for each checkpoint plus JSON containing diff pixels, major-diff pixels, max color delta, unique color counts, palette/DMA/KEY1/CGB-attribute metrics, and per-stage pass/fail status. The current threshold is intentionally tolerant because GBemu's CGB color conversion and FIFO timing are not pixel-perfect yet; exact diff counts are evidence, not a compatibility claim.
...
- Performance gate: text `run_fps=89.23`; sprites `run_fps=70.99`; sprites with headless audio output `run_fps=61.39`, `apu_dropped_samples=0`.
- Super Mario Land action gate: headless action `run_fps=76.07`; action with headless audio output `run_fps=64.67`, `apu_dropped_samples=0`. Latest live action capture checked 10 non-startup profile windows with min `wall_fps=46.84`, min queue `33.5 ms`, and zero audio underruns/drops.
- CGB foundation/render smoke: synthetic header/mode/register checks pass for CGB detection, DMG inert behavior, forced CGB-only startup, VRAM/WRAM bank selects, CGB palettes, OPRI, GDMA/HDMA, and KEY1 STOP-triggered double-speed switching. Synthetic double-speed checks verify TIMA/serial/OAM DMA stay on the CPU-speed domain while PPU/APU/HDMA stay on the normal-speed device domain. Local Pokemon Crystal smoke detects `CGB only ($C0)`, default `Mode: CGB`, `--mode auto` `Mode: CGB`, CPU `A=$11`, and a staged 60/600/2400/3600-frame CGB render gate. Latest staged evidence: 60 frames has 3 unique RGB colors, 32 HDMA blocks / 512 VRAM-DMA bytes, and no KEY1 arm/toggle; 600 frames is a solid transition frame but keeps RGB output, nonzero palettes, and CGB mode; 2400 frames has 7 unique RGB colors plus palette, tile-bank, X-flip, and Y-flip attributes; 3600 frames has 8 unique RGB colors, 20 nonzero attrs, 12 bank attrs, 12 X-flip attrs, and 12 Y-flip attrs. Synthetic CGB renderer checks also cover OBJ palette selection, OBJ tile VRAM-bank selection, OAM-order priority, OPRI DMG-style priority, BG priority attributes, and LCDC.0 priority behavior.
- Crystal PyBoy CGB visual oracle: staged 60/600/2400/3600/4800-frame comparison passes with 15 PNG artifacts. Current exact diff ratios are high where color/FIFO behavior is still approximate: frame 60 `diff_ratio=1.0000`, frame 2400 `0.6257`, frame 3600 `0.3599`, frame 4800 `1.0000`; major-diff ratios under the tolerant threshold are `0.0000`, `0.0174`, `0.0001`, `0.1914`, and `0.4483`.
...
- CGB foundation is started, and CGB-only cartridges now enter CGB mode with basic post-boot identity, KEY1 STOP-triggered double-speed switching, GDMA/HDMA VRAM DMA, first-pass BG/window palette/tile-attribute rendering, first-pass OBJ palette/priority rendering, staged Crystal metrics, and a tolerant PyBoy CGB visual oracle. Exact STOP speed-switch blackout timing, exact GDMA/HDMA CPU-stall timing, exact CGB FIFO/raster timing, color accuracy, full CGB boot flow, and broad CGB game compatibility are not implemented yet.
