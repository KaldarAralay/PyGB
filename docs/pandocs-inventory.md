...
| Gate | Current result |
| --- | --- |
| Unit suite | `396 tests`, `OK` on 2026-05-24. |
...
| Automated Pokemon Red performance gate | `scripts\verify_pokemon_red_performance.py`: text `run_fps=87.29`; sprites `run_fps=70.18`; sprites with headless audio output `run_fps=61.84`, `apu_dropped_samples=0`; deterministic frame/instruction/cycle totals matched exactly. |
| Super Mario Land action gate | `scripts\verify_super_mario_land_performance.py`: action `run_fps=74.87`; action with headless audio output `run_fps=63.50`, `apu_dropped_samples=0`; live action capture min `wall_fps=46.84`, min queue `33.5 ms`, and zero audio underruns/drops. |
...
| Pokemon Crystal CGB render gate | `scripts\verify_crystal_cgb_render.py`: staged `60,600,2400,3600` lane keeps Crystal in CGB mode, writes deterministic JSON metrics, and dumps BMP checkpoints under `qa-output\crystal-cgb-stages`. Latest staged run reports frame 60 with 3 unique RGB colors, 32 HDMA blocks, 512 DMA bytes, and no KEY1 arm/toggle; frame 600 is a solid transition frame with RGB output and nonzero palettes; frame 2400 has 6 unique RGB colors, 607 nonzero attrs, 448 bank attrs, 448 X-flip attrs, and 448 Y-flip attrs; frame 3600 has 8 unique RGB colors, 832 nonzero attrs, 576 bank attrs, 384 X-flip attrs, and 384 Y-flip attrs. Synthetic checks isolate CGB BG palette/bank/flip behavior and CGB OBJ palette, tile-bank, OAM priority, OPRI priority, BG priority, and LCDC.0 priority behavior. |
| Pokemon Crystal PyBoy CGB visual oracle | `scripts\verify_crystal_cgb_oracle.py`: runs GBemu and PyBoy in CGB mode at 60, 600, 2400, 3600, and scripted 4800 wall-frame checkpoints, dumps GBemu/PyBoy/diff/crop PNGs, and writes JSON with diff pixels, max color delta, unique colors, nonblack coverage, and GBemu palette/DMA/KEY1/CGB-attr metrics. Latest run wrote 30 PNGs and passed with tolerant RGB and nonblack structural thresholds. Frame 3600's major-diff ratio dropped to `0.1585` after fixing CPU direct-fast CGB VRAM bank handling; exact diff ratios remain high at several stages, so this is a visual radar rather than a pixel-perfect CGB claim. |
...
| Memory map | `bus.py` implements cartridge ROM/RAM, VRAM, WRAM, echo RAM, OAM, unusable range behavior, IO, HRAM, IE, boot ROM overlay, mode-based VRAM/OAM access restrictions, CGB VRAM/WRAM bank selection, KEY1 double-speed state, and CGB GDMA/HDMA VRAM transfers. | Partial | FEA0-FEFF currently uses a conservative `$FF` behavior rather than revision-specific DMG/CGB quirks; exact STOP speed-switch blackout timing and exact DMA CPU-stall timing are pending. |
...
| CGB registers and mode | `cartridge.py`, `bus.py`, `emulator.py`, `main.py`, `display.py`, and `ppu.py` detect CGB headers, expose `DMG`/`CGB`/`auto` mode selection, force CGB-only cartridges into CGB mode, keep CGB-only IO inert in DMG mode, implement the foundation for `FF4F`, `FF51`-`FF55`, `FF70`, `FF68`-`FF6C` including blocked palette data reads/writes during mode 3 with auto-increment preserved, KEY1 `FF4D` arming/readback/STOP switching, and double-speed CPU/device timing domains, render first-pass CGB BG/window palettes and tile attributes plus CGB OBJ palette/priority rules, and verify Crystal first-frame/window/staged-render/PyBoy-oracle smoke. | Partial foundation | Exact STOP speed-switch blackout timing, exact GDMA/HDMA CPU-stall timing, full CGB boot behavior, broader CGB render oracles, and broad CGB game compatibility remain pending. |
