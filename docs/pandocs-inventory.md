...
| Gate | Current result |
| --- | --- |
| Unit suite | `393 tests`, `OK` on 2026-05-24. |
...
| Automated Pokemon Red performance gate | `scripts\verify_pokemon_red_performance.py`: text `run_fps=89.23`; sprites `run_fps=70.99`; sprites with headless audio output `run_fps=61.39`, `apu_dropped_samples=0`; deterministic frame/instruction/cycle totals matched exactly. |
| Super Mario Land action gate | `scripts\verify_super_mario_land_performance.py`: action `run_fps=76.07`; action with headless audio output `run_fps=64.67`, `apu_dropped_samples=0`; live action capture min `wall_fps=46.84`, min queue `33.5 ms`, and zero audio underruns/drops. |
| CGB foundation smoke | `scripts\verify_cgb_foundation.py`: synthetic checks pass for CGB headers, default DMG behavior, forced CGB-only startup, explicit/auto CGB mode, CGB post-boot `A=$11`, VRAM/WRAM bank selects, GDMA/HDMA, palette RAM, OPRI, KEY1 arming/readback/STOP switching, and double-speed timing-domain behavior. Local Pokemon Crystal smoke detects `PM_CRYSTAL` as CGB-only and confirms default/auto CLI `Mode: CGB`. |
...
| Pokemon Crystal CGB render gate | `scripts\verify_crystal_cgb_render.py`: staged `60,600,2400,3600` lane keeps Crystal in CGB mode, writes deterministic JSON metrics, and dumps BMP checkpoints under `qa-output\crystal-cgb-stages`. Latest staged run reports frame 60 with 3 unique RGB colors, 32 HDMA blocks, 512 DMA bytes, and no KEY1 arm/toggle; frame 600 is a solid transition frame with RGB output and nonzero palettes; frame 2400 has 7 unique RGB colors and palette/tile-bank/X-flip/Y-flip attrs; frame 3600 has 8 unique RGB colors, 20 nonzero attrs, 12 bank attrs, 12 X-flip attrs, and 12 Y-flip attrs. Synthetic checks isolate CGB BG palette/bank/flip behavior and CGB OBJ palette, tile-bank, OAM priority, OPRI priority, BG priority, and LCDC.0 priority behavior. |
| Pokemon Crystal PyBoy CGB visual oracle | `scripts\verify_crystal_cgb_oracle.py`: runs GBemu and PyBoy in CGB mode at 60, 600, 2400, 3600, and scripted 4800-frame checkpoints, dumps GBemu/PyBoy/diff PNGs, and writes JSON with diff pixels, max color delta, unique colors, and GBemu palette/DMA/KEY1/CGB-attr metrics. Latest run wrote 15 PNGs and passed with a tolerant `major_delta_threshold=224`; exact diff ratios remain high at several stages, so this is a visual radar rather than a pixel-perfect CGB claim. |
