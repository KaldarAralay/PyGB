...
| Pokemon Red automated performance gate | Pass | `python -B scripts\verify_pokemon_red_performance.py --json-output qa-output\pokemon-red-performance-gate.json` | Latest run: text `run_fps=85.98`; sprites `run_fps=68.99`; sprites with headless audio output `run_fps=60.07`, `apu_dropped_samples=0`; deterministic frame/instruction/cycle totals matched exactly. |
...
| Super Mario Land early-action performance gate | Pass | `python -B scripts\verify_super_mario_land_performance.py --json-output qa-output\super-mario-land-performance-gate.json` | Latest headless run: action `run_fps=73.81`; action with headless audio output `run_fps=63.25`, `apu_dropped_samples=0`; deterministic frame/instruction/cycle totals matched exactly. Latest live capture checked 10 non-startup windows with min `wall_fps=46.84`, min queue `33.5 ms`, and zero audio underruns/drops. |
...
| Pokemon Crystal saved-game overworld CGB PyBoy oracle | Pass with tolerant threshold | `python -B scripts\verify_crystal_cgb_oracle.py --scenario overworld --output-dir qa-output\crystal-cgb-pyboy-oracle-overworld --json-output qa-output\crystal-cgb-pyboy-oracle-overworld.json` | Loads `saves\pokemon-crystal-test.sav` in both GBemu and PyBoy, continues into a saved-game overworld scene, moves through NPC/object-heavy screens, opens/closes the menu, and captures NPC text through frame 11400. Latest run passes all 11 checkpoints. Frame 4800 is exact; frames 5400, 6000, 6600, 7200, 7800, 8400, 9000, 9600, 10200, 10800, and 11400 are structurally stable but non-exact, with mismatch classification `palette`. Major diff pixels stay at `0` except frames 5400 `79`, 6000 `69`, and 7200 `105`; nonblack coverage deltas are `0.0000` except frames 5400 `0.0002` and 6000 `0.0016`. This is the first saved-game Crystal gameplay oracle and points the next CGB work at color/palette accuracy rather than missing BG/window/OBJ structure. |
...
- Current automated performance gate:
  - Text scene: 240 frames, `run_fps=85.98`, `cpu_instr=1564703`, `cpu_cycles=16853764`.
  - Sprite-heavy scene: 600 frames, `run_fps=68.99`, `cpu_instr=2717563`, `cpu_cycles=42134400`, `ppu_max_sprites=10`, `ppu_sprite_lines=19200`.
  - Sprite-heavy scene with headless audio output: 600 frames, `run_fps=60.07`, `apu_samples=443012`, `apu_dropped_samples=0`, same deterministic CPU totals.
...
- Headless action scene: 600 frames, `run_fps=73.81`, `cpu_instr=1343492`, `cpu_cycles=42379808`, `ppu_max_sprites=4`, `ppu_sprite_lines=9929`.
- Headless action scene with audio output: 600 frames, `run_fps=63.25`, `apu_samples=445592`, `apu_dropped_samples=0`, same deterministic CPU totals.
