...
| --- | --- | --- | --- |
| Unit test suite | Pass | `.\.tools\python-3.12.4-embed-amd64\python.exe -B -m unittest discover -v` | Latest run on 2026-05-24: 396 tests passed. Covers CPU, bus/timers, cartridge mappers, runtime, display/window profiling, joypad, PPU, APU/audio, CGB foundation/startup/render behavior, CGB palette mode-3 access, CGB VRAM DMA, CGB direct-fast VRAM bank handling, CGB double-speed timing domains, CGB OBJ palette/priority behavior, `dmg-acid2`, exact-vs-fast shadow-OAM helper coverage, performance-gate parsing, Crystal render-gate parsing/assertions, Crystal PyBoy oracle diff/evaluation helpers, and APU verifier parsing. |
...
| Pokemon Red automated performance gate | Pass | `python -B scripts\verify_pokemon_red_performance.py --json-output qa-output\pokemon-red-performance-gate.json` | Latest run: text `run_fps=87.29`; sprites `run_fps=70.18`; sprites with headless audio output `run_fps=61.84`, `apu_dropped_samples=0`; deterministic frame/instruction/cycle totals matched exactly. |
...
| Super Mario Land early-action performance gate | Pass | `python -B scripts\verify_super_mario_land_performance.py --json-output qa-output\super-mario-land-performance-gate.json` | Latest headless run: action `run_fps=74.87`; action with headless audio output `run_fps=63.50`, `apu_dropped_samples=0`; deterministic frame/instruction/cycle totals matched exactly. Latest live capture checked 10 non-startup windows with min `wall_fps=46.84`, min queue `33.5 ms`, and zero audio underruns/drops. |
...
| Pokemon Crystal PyBoy CGB visual oracle | Pass with tolerant threshold | `python -B scripts\verify_crystal_cgb_oracle.py --output-dir qa-output\crystal-cgb-pyboy-oracle --json-output qa-output\crystal-cgb-pyboy-oracle.json` | Runs GBemu and PyBoy in CGB mode at 60, 600, 2400, 3600, and scripted 4800 wall-frame checkpoints, writes GBemu/PyBoy/diff/crop PNGs, and records palette/DMA/KEY1/CGB-attr plus nonblack coverage metrics. Latest run wrote 30 PNGs and passed with `major_delta_threshold=224`, `max_major_diff_ratio=0.95`, `max_nonblack_delta_ratio=0.98`. Exact diff remains high because GBemu's CGB color/FIFO behavior is approximate: frame 60 `diff_ratio=1.0000`, frame 600 `0.0379`, frame 2400 `0.6257`, frame 3600 `0.3335`, frame 4800 `1.0000`; major-diff ratios are now `0.0000`, `0.0174`, `0.0017`, `0.1585`, and `0.3883`. Nonblack coverage deltas were `1.0000`, `0.5709`, `0.0088`, `0.4721`, and `0.0859`. Frame 60 is a startup/LCD-transition sample; later checkpoints are structural radar. Frame 3600's fixed class was wrong CGB VRAM bank selection in CPU direct-fast VRAM access, which had suppressed bank-1 tile attributes. |
...
- Current automated performance gate:
  - Text scene: 240 frames, `run_fps=87.29`, `cpu_instr=1564703`, `cpu_cycles=16853764`.
  - Sprite-heavy scene: 600 frames, `run_fps=70.18`, `cpu_instr=2717563`, `cpu_cycles=42134400`, `ppu_max_sprites=10`, `ppu_sprite_lines=19200`.
  - Sprite-heavy scene with headless audio output: 600 frames, `run_fps=61.84`, `apu_samples=443012`, `apu_dropped_samples=0`, same deterministic CPU totals.
...
- Headless action scene: 600 frames, `run_fps=74.87`, `cpu_instr=1343492`, `cpu_cycles=42379808`, `ppu_max_sprites=4`, `ppu_sprite_lines=9929`.
- Headless action scene with audio output: 600 frames, `run_fps=63.50`, `apu_samples=445592`, `apu_dropped_samples=0`, same deterministic CPU totals.
