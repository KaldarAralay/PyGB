...
| Pokemon Red automated performance gate | Pass | `python -B scripts\verify_pokemon_red_performance.py --json-output qa-output\pokemon-red-performance-gate.json` | Latest run: text `run_fps=86.15`; sprites `run_fps=70.28`; sprites with headless audio output `run_fps=61.10`, `apu_dropped_samples=0`; deterministic frame/instruction/cycle totals matched exactly. |
...
| Super Mario Land early-action performance gate | Pass | `python -B scripts\verify_super_mario_land_performance.py --json-output qa-output\super-mario-land-performance-gate.json` | Latest headless run: action `run_fps=73.90`; action with headless audio output `run_fps=62.55`, `apu_dropped_samples=0`; deterministic frame/instruction/cycle totals matched exactly. Latest live capture checked 10 non-startup windows with min `wall_fps=46.84`, min queue `33.5 ms`, and zero audio underruns/drops. |
...
| Pokemon Crystal saved-game overworld CGB PyBoy oracle | Pass with tolerant threshold | `python -B scripts\verify_crystal_cgb_oracle.py --scenario overworld --output-dir qa-output\crystal-cgb-pyboy-oracle-overworld --json-output qa-output\crystal-cgb-pyboy-oracle-overworld.json` | Loads `saves\pokemon-crystal-test.sav` in both GBemu and PyBoy, converts the GBemu JSON RTC sidecar into PyBoy's RTC format, freezes both emulators to `saved_at=1779714125.565259`, continues into a saved-game overworld scene, moves through NPC/object-heavy screens, opens/closes the menu, and captures NPC text through frame 11400. Latest run passes 10 checkpoints: 4800, 5400, 7200, 8400, 8580, 9060, 9600, 10200, 10800, and 11400. Frames 4800, 8580, 9060, 9600, 10200, and 10800 are exact. Frames 5400, 7200, 8400, and 11400 have small transient `obj_priority`/OBJ coverage diffs of 75-78 pixels with nonblack coverage delta at or below `0.0017`. BG and OBJ palette RAM match PyBoy at every source-debug checkpoint, proving the prior saved-game palette drift was RTC/time-of-day baseline drift rather than RGB555 conversion, mode-3 palette blocking, or palette RAM state. Focused 5400/8400/9600 source-debug classifies the active focus drift as OAM/transition timing: first OAM diffs are tile `100` vs `96` at offset `0x22` for frame 5400 and tile `104` vs `100` at offset `0x12` for frame 8400; frame 9600 is exact. |
...
- Current automated performance gate:
  - Text scene: 240 frames, `run_fps=86.15`, `cpu_instr=1564703`, `cpu_cycles=16853764`.
  - Sprite-heavy scene: 600 frames, `run_fps=70.28`, `cpu_instr=2717563`, `cpu_cycles=42134400`, `ppu_max_sprites=10`, `ppu_sprite_lines=19200`.
  - Sprite-heavy scene with headless audio output: 600 frames, `run_fps=61.10`, `apu_samples=443012`, `apu_dropped_samples=0`, same deterministic CPU totals.
...
- Headless action scene: 600 frames, `run_fps=73.90`, `cpu_instr=1343492`, `cpu_cycles=42379808`, `ppu_max_sprites=4`, `ppu_sprite_lines=9929`.
- Headless action scene with audio output: 600 frames, `run_fps=62.55`, `apu_samples=445592`, `apu_dropped_samples=0`, same deterministic CPU totals.
...
The current codebase was compared against the major Pan Docs hardware areas on 2026-05-25. The detailed inventory is in `docs\pandocs-inventory.md`.
