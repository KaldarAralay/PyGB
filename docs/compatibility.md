...
| --- | --- | --- | --- |
| Unit test suite | Pass | `.\.tools\python-3.12.4-embed-amd64\python.exe -B -m unittest discover -v` | Latest run on 2026-05-26: 427 tests passed. Covers CPU, bus/timers, cartridge mappers, runtime, display/window profiling, joypad, PPU, APU/audio, CGB foundation/startup/render behavior, CGB palette mode-3 access, CGB VRAM DMA, CGB fast WRAM bank handling, CGB double-speed timing domains, CGB OBJ palette/priority behavior, `dmg-acid2`, exact-vs-fast shadow-OAM helper coverage, performance-gate parsing, Crystal render-gate parsing/assertions, Crystal PyBoy oracle scenario/source-debug/mismatch-class helpers, saved-game oracle save-file/RTC selection, Crystal performance-gate parsing/evaluation, and APU verifier parsing. |
...
| Pokemon Red automated performance gate | Pass | `python -B scripts\verify_pokemon_red_performance.py --json-output qa-output\pokemon-red-performance-final-baseline.json` | Latest run: text `run_fps=98.12`; sprites `run_fps=82.15`; sprites with headless audio output `run_fps=66.03`, `apu_samples=443012`, `apu_dropped_samples=0`; deterministic frame/instruction/cycle totals matched exactly. |
...
| Super Mario Land early-action performance gate | Pass | `python -B scripts\verify_super_mario_land_performance.py --json-output qa-output\super-mario-land-performance-final-baseline.json` | Latest headless run: action `run_fps=92.25`; action with headless audio output `run_fps=75.51`, `apu_samples=445592`, `apu_dropped_samples=0`; deterministic frame/instruction/cycle totals matched exactly. Latest live capture checked 10 non-startup windows with min `wall_fps=46.84`, min queue `33.5 ms`, and zero audio underruns/drops. |
| CGB foundation smoke | Pass | `.\.tools\python-3.12.4-embed-amd64\python.exe -B scripts\verify_cgb_foundation.py --json-output qa-output\cgb-foundation.json` | Synthetic smoke verifies CGB header detection, explicit/auto mode selection, forced CGB-only startup, DMG inert behavior, `FF4F` VRAM bank select, `FF70` WRAM bank select, `FF51`-`FF55` GDMA/HDMA behavior, `FF68`-`FF6C` palette/OPRI registers including blocked palette data reads/writes during mode 3, KEY1 arming/readback/STOP switching, and double-speed timer/PPU/APU timing-domain behavior. Local Crystal smoke detects `PM_CRYSTAL` as `CGB only` and confirms default and `--mode auto` CLI output `Mode: CGB`. This is not a CGB game compatibility claim. |
...
| Pokemon Crystal CGB render gate | Pass | `.\.tools\python-3.12.4-embed-amd64\python.exe -B scripts\verify_crystal_cgb_render.py --checkpoint-frames 60,600,2400,3600 --require-crystal-attributes --json-output qa-output\crystal-cgb-render-staged.json --frame-output-dir qa-output\crystal-cgb-stages` | Staged lane keeps Crystal in CGB mode, writes JSON metrics, and dumps BMPs for 60, 600, 2400, and 3600 frames. Latest staged run: frame 60 has 3 unique RGB colors, 32 HDMA blocks, 512 DMA bytes, and no KEY1 arm/toggle; frame 600 is a solid transition frame with RGB output and nonzero palettes; frame 2400 has 6 unique RGB colors, 607 nonzero attrs, 448 bank attrs, 448 X-flip attrs, and 448 Y-flip attrs; frame 3600 has 18 unique RGB colors, 832 nonzero attrs, 576 bank attrs, 384 X-flip attrs, and 384 Y-flip attrs. Synthetic checks verify CGB BG palette/bank/flip behavior plus CGB OBJ palette selection, OBJ tile VRAM-bank selection, OAM-order priority, OPRI DMG-style priority, BG priority attributes, and LCDC.0 priority behavior. This is still a staged render gate, not a Crystal compatibility claim. |
| Pokemon Crystal PyBoy CGB visual oracle | Pass with tolerant threshold | `python -B scripts\verify_crystal_cgb_oracle.py --output-dir qa-output\crystal-cgb-pyboy-oracle --json-output qa-output\crystal-cgb-pyboy-oracle.json` | Runs GBemu and PyBoy in CGB mode at 60, 600, 2400, 3600, and scripted 4800 wall-frame checkpoints, writes GBemu/PyBoy/diff/crop PNGs, and records palette/DMA/KEY1/CGB-attr plus nonblack coverage metrics. Source-debug JSON classifies BG/window/OBJ source, tile id, attr byte, palette, color id, priority, visible mismatch class, source-state class, and mismatch class. Latest run wrote 30 PNGs and passed with `major_delta_threshold=224`, `max_major_diff_ratio=0.95`, `max_nonblack_delta_ratio=0.98`. The old frame-3600 visible coverage mismatch was bank-0 BG tilemap transfer timing caused by CPU fast WRAM helpers bypassing selected CGB WRAM bank `$D000-$DFFF` and echo RAM; fixing that path made frame 2400, 3600, and 4800 pixel-exact. Current diff ratios are: frame 60 `1.0000`, frame 600 `0.0174`, frame 2400 `0.0000`, frame 3600 `0.0000`, frame 4800 `0.0000`; major-diff ratios are `0.0000`, `0.0174`, `0.0000`, `0.0000`, and `0.0000`; nonblack coverage deltas are `1.0000`, `0.4577`, `0.0000`, `0.0000`, and `0.0000`. Frame 3600's source-debug reports `visible_mismatch_class=none` and confirms OAM, LCDC/SCX/SCY/WX/WY, palette RAM, and bank-1 attr maps match PyBoy; an invisible source-state drift remains as 48 differing bytes in bank-0 `9800` tilemap state. |
| Pokemon Crystal dynamic CGB PyBoy oracle | Pass | `python -B scripts\verify_crystal_cgb_oracle.py --scenario dynamic --output-dir qa-output\crystal-cgb-pyboy-oracle-dynamic --json-output qa-output\crystal-cgb-pyboy-oracle-dynamic.json` | Adds deterministic checkpoints for title animation/palette, logo transition, static title lock, gender-menu text, cursor-down movement, cursor-up movement, dialog transition, intro dialog text, clock day menu, clock minute menu, and clock confirmation menu through frame 7800. All 11 current dynamic checkpoints pass with `diff_ratio=0.0000`, `major_diff_ratio=0.0000`, `nonblack_delta_ratio=0.0000`, `visible_mismatch_class=none`, and `mismatch_class=none`. Source-debug is emitted at every dynamic checkpoint so future mismatches are classified into palette, CGB attr, OBJ priority, HDMA timing, window timing, BG/window tilemap, VRAM bank, FIFO timing, or no visible mismatch. |
| Pokemon Crystal saved-game overworld CGB PyBoy oracle | Pass | `python -B scripts\verify_crystal_cgb_oracle.py --scenario overworld --checkpoint-frames 5400,8400,9600 --source-debug-checkpoints 5400,8400,9600 --strict-source-state --output-dir qa-output\crystal-cgb-oracle-strict-final-baseline --json-output qa-output\crystal-cgb-oracle-strict-final-baseline.json` | Loads `saves\pokemon-crystal-test.sav` in both GBemu and PyBoy, converts the GBemu JSON RTC sidecar into PyBoy's RTC format, freezes both emulators to `saved_at=1779757132.7976005`, and verifies the strict saved-game source-state focus at checkpoints 5400, 8400, and 9600. Latest bounded pass: all three checkpoints are pixel-exact with `diff_pixels=0`, `major_diff_pixels=0`, `nonblack_delta_ratio=0.0000`, and `mismatch_class=none`. Broader 10-checkpoint overworld evidence remains staged, not a broad Crystal compatibility claim. |
| Pokemon Crystal saved-game performance gate | Pass | `python -B scripts\verify_pokemon_crystal_performance.py --json-output qa-output\crystal-performance-gate-final-baseline.json` | Uses the deterministic `saves\pokemon-crystal-test.sav` plus halted `.sav.rtc` fixture and fixed wall-frame input. Latest headless run: overworld `run_fps=65.51`, worst 60-frame window `64.31`; overworld with headless audio output `run_fps=55.07`, worst 60-frame window `53.43`, `apu_samples=443012`, `apu_dropped_samples=0`. Both lanes match `frames=600`, `ppu_frames=600`, `cpu_instr=1624904`, and `cpu_cycles=42134400`. Live-window gate latest run: 100 profile windows captured, 8 measured after warmup, min `wall_fps=46.04`, average `47.17`, min queue `33.3 ms`, and zero audio underruns/drops. Crystal CGB gameplay performance is good enough for now; further work should move to CGB FIFO/raster correctness. |
...
- Current automated performance gate:
  - Text scene: 240 frames, `run_fps=98.12`, `cpu_instr=1564703`, `cpu_cycles=16853764`.
  - Sprite-heavy scene: 600 frames, `run_fps=82.15`, `cpu_instr=2717563`, `cpu_cycles=42134400`, `ppu_max_sprites=10`, `ppu_sprite_lines=19200`.
  - Sprite-heavy scene with headless audio output: 600 frames, `run_fps=66.03`, `apu_samples=443012`, `apu_dropped_samples=0`, same deterministic CPU totals.
...
- Headless action scene: 600 frames, `run_fps=92.25`, `cpu_instr=1343492`, `cpu_cycles=42379808`, `ppu_max_sprites=4`, `ppu_sprite_lines=9929`.
- Headless action scene with audio output: 600 frames, `run_fps=75.51`, `apu_samples=445592`, `apu_dropped_samples=0`, same deterministic CPU totals.
...
| Memory bus and timers | Partial | DIV/TIMA edge ticking, delayed TIMA reload, STOP timer freeze, serial transfer timing, OAM DMA bus blocking and line visibility, boot ROM overlay, IO read masks, CGB-only DMG inert registers, forced CGB-only startup, CGB foundation banking/register storage, CPU fast-path CGB WRAM bank selection, CGB palette data mode-3 access blocking, KEY1 STOP-triggered double-speed switching, CGB CPU-speed timing for DIV/TIMA/serial/OAM DMA, normal-speed timing for PPU/APU/HDMA, and CGB GDMA/HDMA VRAM DMA data movement. | More hardware timing tests are needed for uncommon write-ordering and interrupt-boundary cases; exact STOP speed-switch blackout timing and exact GDMA/HDMA CPU-stall timing are not implemented. |
| PPU and framebuffer | Partial | LCD modes, `LY`/`STAT`, VBlank, line-153 wrap, DMG STAT quirk, DMG BG/window/OBJ rendering, first-pass CGB BG/window palette and tile-attribute rendering, first-pass CGB OBJ palette and priority rendering, RGB framebuffer export/display, palette handling, scroll wrapping, sprite priority, mode-3 penalties, selected segmented mid-line effects, OAM DMA sprite hiding, `dmg-acid2`, Mooneye acceptance/ppu, selected Mealybug image cases, and static/dynamic/saved-game-overworld Pokemon Crystal PyBoy CGB visual oracles. | Full per-dot FIFO behavior, every mid-scanline raster edge, broader PPU ROM suites, CGB raster timing, and stricter CGB commercial-game visual oracles remain pending. |
...
| Runtime/display | Partial | Frame stepping, stop conditions, save lifecycle, reset preserving cartridge state, frame dumps, Tkinter window mode, keyboard controls, live audio, trace toggle, frame pacing, rolling spike profiling, and CGB first-frame startup diagnostics. | Tkinter pacing is good enough for current Pokemon Red and Pokemon Crystal testing but remains host/runtime dependent. |
| Performance | Partial | Pokemon Red heavy windows are optimized with guarded hot paths and verified with instruction/cycle/audio identity checks for the covered runs. Super Mario Land adds a quick early-action performance smoke gate with headless and live-profile evidence. Pokemon Crystal has a deterministic saved-game CGB overworld performance gate with per-window FPS, CPU/frame drift, APU sample-drop, PPU/DMA counters, and a passing live-window gate. | Crystal performance is frozen as good enough for now. Broader ROMs may expose different hot paths, but the next major workstream should be CGB FIFO/raster correctness rather than more FPS tuning. |

## Pan Docs Inventory

The current codebase was compared against the major Pan Docs hardware areas on 2026-05-26. The detailed inventory is in `docs\pandocs-inventory.md`.
...
| CGB and SGB | CGB foundation plus KEY1 double-speed switching, GDMA/HDMA, first-pass BG/window palette/tile-attribute rendering, and OBJ palette/priority rendering started; SGB not implemented. |
