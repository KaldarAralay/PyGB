...
| --- | --- | --- | --- |
| Unit test suite | Pass | `.\.tools\python-3.12.4-embed-amd64\python.exe -B -m unittest discover -v` | Latest run on 2026-05-23: 361 tests passed. Covers CPU, bus/timers, cartridge mappers, runtime, display/window profiling, joypad, PPU, APU/audio, CGB foundation/startup behavior, `dmg-acid2`, exact-vs-fast shadow-OAM helper coverage, performance-gate parsing, and APU verifier parsing. |
...
| Pokemon Red automated performance gate | Pass | `python -B scripts\verify_pokemon_red_performance.py --json-output qa-output\pokemon-red-performance-gate.json` | Latest run: text `run_fps=91.91`; sprites `run_fps=75.01`; sprites with headless audio output `run_fps=64.98`, `apu_dropped_samples=0`; deterministic frame/instruction/cycle totals matched exactly. |
...
- Current automated performance gate:
  - Text scene: 240 frames, `run_fps=91.91`, `cpu_instr=1564703`, `cpu_cycles=16853764`.
  - Sprite-heavy scene: 600 frames, `run_fps=75.01`, `cpu_instr=2717563`, `cpu_cycles=42134400`, `ppu_max_sprites=10`, `ppu_sprite_lines=19200`.
  - Sprite-heavy scene with headless audio output: 600 frames, `run_fps=64.98`, `apu_samples=443012`, `apu_dropped_samples=0`, same deterministic CPU totals.
...
- Headless action scene: 600 frames, `run_fps=79.65`, `cpu_instr=1343492`, `cpu_cycles=42379808`, `ppu_max_sprites=4`, `ppu_sprite_lines=9929`.
- Headless action scene with audio output: 600 frames, `run_fps=67.08`, `apu_samples=445592`, `apu_dropped_samples=0`, same deterministic CPU totals.
...
| CGB and SGB | CGB foundation started; SGB not implemented. |
