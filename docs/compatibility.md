...
| Pokemon Crystal CGB render smoke | Pass | `.\.tools\python-3.12.4-embed-amd64\python.exe -B scripts\verify_crystal_cgb_render.py --json-output qa-output\crystal-cgb-render.json`; optional `--frames 2400 --require-crystal-attributes` | Quick lane reaches 60 Crystal frames with a full RGB framebuffer, 3 unique colors, nonzero BG palette RAM, and required CGB VRAM-DMA activity. Latest quick lane: 32 HDMA blocks, 512 DMA bytes, `speed_switch_arm_writes=0`, and `speed_switches=0`. The longer lane reached 2400 frames with palette, tile-bank, X-flip, and Y-flip attributes present. Synthetic checks verify CGB BG palette/bank/flip behavior plus CGB OBJ palette selection, OBJ tile VRAM-bank selection, OAM-order priority, OPRI DMG-style priority, BG priority attributes, and LCDC.0 priority behavior. This is not a Crystal compatibility claim. |
