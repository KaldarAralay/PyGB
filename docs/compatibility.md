...
| --- | --- | --- | --- |
| Unit test suite | Pass | `.\.tools\python-3.12.4-embed-amd64\python.exe -B -m unittest discover -v` | Latest run on 2026-05-24: 393 tests passed. Covers CPU, bus/timers, cartridge mappers, runtime, display/window profiling, joypad, PPU, APU/audio, CGB foundation/startup/render behavior, CGB VRAM DMA, CGB double-speed timing domains, CGB OBJ palette/priority behavior, `dmg-acid2`, exact-vs-fast shadow-OAM helper coverage, performance-gate parsing, Crystal render-gate parsing/assertions, Crystal PyBoy oracle diff/evaluation helpers, and APU verifier parsing. |
...
| Pokemon Red automated performance gate | Pass | `python -B scripts\verify_pokemon_red_performance.py --json-output qa-output\pokemon-red-performance-gate.json` | Latest run: text `run_fps=89.23`; sprites `run_fps=70.99`; sprites with headless audio output `run_fps=61.39`, `apu_dropped_samples=0`; deterministic frame/instruction/cycle totals matched exactly. |
...
| Super Mario Land early-action performance gate | Pass | `python -B scripts\verify_super_mario_land_performance.py --json-output qa-output\super-mario-land-performance-gate.json` | Latest headless run: action `run_fps=76.07`; action with headless audio output `run_fps=64.67`, `apu_dropped_samples=0`; deterministic frame/instruction/cycle totals matched exactly. Latest live capture checked 10 non-startup windows with min `wall_fps=46.84`, min queue `33.5 ms`, and zero audio underruns/drops. |
| CGB foundation smoke | Pass | `.\.tools\python-3.12.4-embed-amd64\python.exe -B scripts\verify_cgb_foundation.py --json-output qa-output\cgb-foundation.json` | Synthetic smoke verifies CGB header detection, explicit/auto mode selection, forced CGB-only startup, DMG inert behavior, `FF4F` VRAM bank select, `FF70` WRAM bank select, `FF51`-`FF55` GDMA/HDMA behavior, `FF68`-`FF6C` palette/OPRI registers, KEY1 arming/readback/STOP switching, and double-speed timer/PPU/APU timing-domain behavior. Local Crystal smoke detects `PM_CRYSTAL` as `CGB only` and confirms default and `--mode auto` CLI output `Mode: CGB`. This is not a CGB game compatibility claim. |
...
| Pokemon Crystal CGB render gate | Pass | `.\.tools\python-3.12.4-embed-amd64\python.exe -B scripts\verify_crystal_cgb_render.py --checkpoint-frames 60,600,2400,3600 --require-crystal-attributes --json-output qa-output\crystal-cgb-render-staged.json --frame-output-dir qa-output\crystal-cgb-stages` | Staged lane keeps Crystal in CGB mode, writes JSON metrics, and dumps BMPs for 60, 600, 2400, and 3600 frames. Latest staged run: frame 60 has 3 unique RGB colors, 32 HDMA blocks, 512 DMA bytes, and no KEY1 arm/toggle; frame 600 is a solid transition frame with RGB output and nonzero palettes; frame 2400 has 7 unique RGB colors and palette/tile-bank/X-flip/Y-flip attrs; frame 3600 has 8 unique RGB colors, 20 nonzero attrs, 12 bank attrs, 12 X-flip attrs, and 12 Y-flip attrs. Synthetic checks verify CGB BG palette/bank/flip behavior plus CGB OBJ palette selection, OBJ tile VRAM-bank selection, OAM-order priority, OPRI DMG-style priority, BG priority attributes, and LCDC.0 priority behavior. This is still a staged render gate, not a Crystal compatibility claim. |
| Pokemon Crystal PyBoy CGB visual oracle | Pass with tolerant threshold | `python -B scripts\verify_crystal_cgb_oracle.py --output-dir qa-output\crystal-cgb-pyboy-oracle --json-output qa-output\crystal-cgb-pyboy-oracle.json` | Runs GBemu and PyBoy in CGB mode at 60, 600, 2400, 3600, and scripted 4800-frame checkpoints, writes GBemu/PyBoy/diff PNGs, and records palette/DMA/KEY1/CGB-attr metrics. Latest run wrote 15 PNGs and passed with `major_delta_threshold=224`, `max_major_diff_ratio=0.95`. Exact diff remains high because GBemu's CGB color/FIFO behavior is approximate: frame 60 `diff_ratio=1.0000`, frame 2400 `0.6257`, frame 3600 `0.3599`, frame 4800 `1.0000`; major-diff ratios were `0.0000`, `0.0174`, `0.0001`, `0.1914`, and `0.4483`. This is a visual radar, not pixel-perfect CGB compatibility. |
...
- Current automated performance gate:
  - Text scene: 240 frames, `run_fps=89.23`, `cpu_instr=1564703`, `cpu_cycles=16853764`.
  - Sprite-heavy scene: 600 frames, `run_fps=70.99`, `cpu_instr=2717563`, `cpu_cycles=42134400`, `ppu_max_sprites=10`, `ppu_sprite_lines=19200`.
  - Sprite-heavy scene with headless audio output: 600 frames, `run_fps=61.39`, `apu_samples=443012`, `apu_dropped_samples=0`, same deterministic CPU totals.
...
- Headless action scene: 600 frames, `run_fps=76.07`, `cpu_instr=1343492`, `cpu_cycles=42379808`, `ppu_max_sprites=4`, `ppu_sprite_lines=9929`.
- Headless action scene with audio output: 600 frames, `run_fps=64.67`, `apu_samples=445592`, `apu_dropped_samples=0`, same deterministic CPU totals.
